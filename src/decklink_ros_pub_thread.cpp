...
bool DeckLinkRosPubThread::init() {
    ...
    camera_info_manager_ = std::make_unique<camera_info_manager::CameraInfoManager>(this->shared_from_this().get(), cname_, url_);
    ...
    decklink_capture_->setCallback(std::bind(
        &DeckLinkRosPubThread::newFrameCallback, 
        this, 
        std::placeholders::_1, 
        std::placeholders::_2, 
        std::placeholders::_3, 
...
void DeckLinkRosPubThread::newFrameCallback(long unsigned frame_count, void* raw_frame, unsigned raw_frame_width, unsigned raw_frame_height, unsigned bytes_per_pixel) {
    if (frame_width_ == 0) {
